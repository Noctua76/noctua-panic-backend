...
test("storage rollback and short-lived tenant-checked signed URLs are present", () => {
  const source = fs.readFileSync(path.join(__dirname, "../reports/shift-reports.js"), "utf8");
  assert.match(source, /storage\.remove\(uploadedPaths\)/);
...
async function withRouter({ pool, storage, guard, admin }, callback) {
  const app = express();
  ...
  app.use(createShiftReportsRouter({
    ...
    requireGuardAuth,
    requireAuth,
    puppeteer: { launch: async () => { throw new Error("PDF not used in this test"); } },
  }));
